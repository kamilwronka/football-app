      ...
      //  this.setState({ currentMatchday });
        ...
        return fixtures.map(elem => {
           if(elem.matchday === Number(selectedMatchday)) {
                const team1_id = _.last(elem._links.homeTeam.href.split("/"));
                const team2_id = _.last(elem._links.awayTeam.href.split("/"));
                const link1 = `/team/${team1_id}/${elem.homeTeamName}`;
                const link2 = `/team/${team2_id}/${elem.awayTeamName}`;
               const { result } = elem;
